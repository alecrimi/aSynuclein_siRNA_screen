...
count_C = 0
temp_repeat = 0
Data_Frame = pd.read_csv('C:\\sRNA_screen_MDD\Mouse_screen\\Test\\Silencer Select hGenome List.csv') # E:\\files\\Silencer Select hGenome List.csv
...
data_frame_A.to_csv ('C:\\sRNA_screen_MDD\Mouse_screen\\Test\\Library_A.csv', index = False)
data_frame_B.to_csv ('C:\\sRNA_screen_MDD\Mouse_screen\\Test\\Library_B.csv', index = False)
data_frame_C.to_csv ('C:\\sRNA_screen_MDD\Mouse_screen\\Test\\Library_C.csv', index = False)
...
for ind, row in data_frame_A.iterrows():    
    ...
    if well == 'P24':
        ...
        path = 'C:\\sRNA_screen_MDD\Mouse_screen\\Test\\plates\\plate' + '_' + 'P' + '_' + str(no_plates) + '.csv'
...
def process_file (ind):    
    file = "C:\\sRNA_screen_MDD\Mouse_screen\\Test\\plate_P_" + str(ind)+ ".csv"
    df = pd.read_csv (file)
    return df
frames = [ process_file(i) for i in range(1, no_plates+1)]
result = pd.concat(frames)
path = 'C:\\sRNA_screen_MDD\Mouse_screen\\Test\\plate_all.csv' 
